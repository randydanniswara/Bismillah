...
class ProfilController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Profil']))
		{
			...
			if($model->validate()){
	           ...
	           Yii::app()->user->setFlash('update','Profil Anda telah diperbaharui');  
	...
	public function actionUpdate()
	{
		...
		{
			$model->attributes=$_POST['Profil'];
			//echo var_dump($_POST['User']);return;

			$user->password = $_POST['User']['password'];
			$user->samePassword = $_POST['User']['samePassword'];
			$user->validate();

			if($model->validate()){
			   $profil = Profil::model()->find("id=".$id);
			   $link = Yii::app()->basePath.$URL_AVATAR.$profil->avatar;
			  //echo file_exists($link);return;	
			   if (file_exists($link) && strlen($profil->avatar)>0) {
    				unlink($link);         
			   } 
	           $model->avatar = CUploadedFile::getInstance($model, 'avatar');
	           $model->avatar->saveAs(Yii::app()->basePath .$URL_AVATAR."Profil_".$model->id.".".$model->avatar->getExtensionName());
