...
class KegiatanController extends Controller
{
	...
	public function actionIndex()
	{
		...
		if (Yii::app()->user->getRole() == 2) {
			$id_lab = LabUser::model()->find("id_user=".Yii::app()->user->id)->id_lab;
			//echo var_dump(Yii::app()->user->id);return;
			$dataProvider=new CActiveDataProvider('Kegiatan', array(
				'criteria' => array(
					'condition'=>'id_lab='.$id_lab,
				)
			));
		} else $dataProvider = new CActiveDataProvider('Kegiatan');
		$this->render('index',array(
			'dataProvider'=>$dataProvider,
		));
	}
