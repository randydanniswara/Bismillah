...
class DokumenController extends Controller
{
	...
	public function actionId($id){
		$dataProvider=new CActiveDataProvider('Dokumen', array(
				'criteria' => array(
					'condition'=>'id_lab='.$id,
				)
		));
		$this->id = $id;
		$this->render('_id',array(
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Dokumen']))
		{
			...
			if($model->save()) {
				$tahu = $_POST['Kategori'];
				foreach ($tahu as $key) {
					$p = new KategoriDokumen;
					$p->id_dokumentasi = $model->id;
					$p->id_kategori = $key;
					$p->save();
				}
				Yii::app()->user->setFlash('create','Dokumen berhasil dibuat');
				$this->redirect(array('view','id'=>$model->id));
			}
		}
		$list = Kategori::model()->getAll();
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Dokumen']))
		{
			...
			if($model->save()) {
				KategoriDokumen::model()->deleteAll("id_dokumentasi=".$model->id);
				$tahu = $_POST['Kategori'];
				foreach ($tahu as $key) {
					$p = new KategoriDokumen;
					$p->id_dokumentasi = $model->id;
				...
				Yii::app()->user->setFlash('update','Dokumen telah diupdate');
				$this->redirect(array('view','id'=>$model->id));
			}
		}
		$list = Kategori::model()->getAll();
